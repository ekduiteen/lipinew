...
| TTS fallback | ✅ | Graceful degradation | Returns silence if Piper (ne_NP-google-medium for Nepali, en_US for English) fails |
...
| TTS generation | < 500ms | 200–400ms (Piper (ne_NP-google-medium for Nepali, en_US for English)) | ✅ |
...
### Commits
```
4b1f5eb Update docker-compose.yml: Qwen2.5-14B-AWQ and Piper (ne_NP-google-medium for Nepali, en_US for English) in production spec
...
**Verification Scope:** Backend (FastAPI), Frontend (Next.js), Database (PostgreSQL), Cache (Valkey), LLM (vLLM), STT/TTS (Piper (ne_NP-google-medium for Nepali, en_US for English))
